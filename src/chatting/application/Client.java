...
public class Client implements ActionListener {
    ...
    Client()
    {
        ...
        back.addMouseListener(new MouseAdapter(){
         ...
         //If you want to setimages in your project then use ImageIcon
        // Creating an object of image icon
        ImageIcon i4 = new ImageIcon(ClassLoader.getSystemResource("icons/messi.jpg"));
        //For setting or scaling the image
    ...
    public static JPanel formatLabel(String out)
    {
        ...
        public static void main(String[] args)
        {
            ...
            try
            {
                //When server is running on one port number
                //then our client have to run on the same port number
                Socket msg = new Socket("127.0.0.1",6001);
                DataInputStream dis = new DataInputStream(msg.getInputStream());
                dos = new DataOutputStream(msg.getOutputStream());
                
                //An infinite loop is created to keep reading incoming messages from the server.
                 while(true)
                    {
                        al.setLayout(new BorderLayout());
